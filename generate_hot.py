...
def generate_inspirations(all_articles):
    ...
    def wangba_style(topic, source, title, idx=0):
        """网吧信息差 — 大学生视角，口语化，带点吐槽感，像在群里跟兄弟唠嗑"""
        ...
        if io:
            patterns = [
                f"兄弟们，今天刷到一条新闻给我看愣了：{s}。我一开始以为是营销号编的，结果上微博一搜，官方通报都出来了。真的就是真的。你们也别急着站队，听我捋一下这个事的时间线。最开始是——然后突然——到了今天——说实话我现在也没缓过来。评论区有没有懂哥？",
                f"巴沙今天本来不想发视频的，结果刷到这条人傻了。{s}。你们知道这件事最离谱的是什么吗？不是事情本身，是事发之后一堆人跳出来说「我早就说过」——你早说过你倒是早说啊！我就想问一句：这条新闻底下，你们觉得谁最冤？",
                f"不说了，直接上新闻。{s}。刷到这条的时候我正在吃饭，筷子都放下了。看了一眼评论区，已经吵成三派了：有说能理解的，有说绝对不能忍的，还有纯吃瓜等反转的。巴沙的态度是——你先别急着站，让子弹飞一会儿。评论区说说你的第一反应。",
                f"今天这条新闻，我建议你先深呼吸再看。{s}。我早上起床打开热搜，前十里面两条都是关于这个的。你想啊，从去年到现在，这种事不是第一次了，每次都能爆。为什么？因为大家都憋着一股劲。什么时候这股劲能过去？不知道。你们品。",
            ]
        elif ip:
            patterns = [
                f"巴沙今天必须给你们发一条好消息：{s}。说实话我一开始在热搜上看到的时候还以为是标题党——后来打开一看，不是，是实打实的好事。这种新闻就该多上热搜，别天天都是那些负面的。评论区刷个「正能量」让巴沙看看有多少人跟我一样高兴。",
                f"早上刷到一条新闻，看完心情好了一整天：{s}。说真的，以前这种事只能在新闻联播里看到，但是这次不太一样——这次是实打实的，有视频有真相。巴沙不多评价，只希望这种新闻多来点。你们看完什么感受？",
            ]
        else:
            patterns = [
                f"来，巴沙跟你们聊一个这两天特别火的事：{s}。我看了不下五个版本的报道，每个版本说的都不太一样。你品，你细品，这里面有一个关键信息被大多数人跳过去了。来评论区，我看看有没有跟我发现同样细节的兄弟。",
                f"今天说一个事，不站队，不评价，纯跟你们唠：{s}。起因不复杂，但是后续的发展——怎么讲呢，电视剧都不敢这么拍。巴沙把各方说辞整理了一下，你们自己品品看：甲方说——、乙方说——、第三方说——。是不是对不上？对不上就对了。评论区你们自己聊。",
                f"热点来了各位。{s}。你说这算不算新闻？算吧，好像不算啥大事。不算吧，朋友圈已经刷屏了。巴沙的感觉是——这事单独看没什么，但如果你把它跟前面几个热点连起来看，方向就出来了。我只是提供一个视角，你们自己判断。",
                f"巴沙今天说一个你们可能都刷到了但是没细想的事：{s}。最迷惑的不是事情本身，是评论区。一边说对，一边说错，中间还夹着几个趁机卖课的。巴沙就想说——能不能让事情单纯一点？就事论事，别扯有的没的。你们觉得呢？",
            ]
        return pick(patterns, topic + 'wb_v7' + str(idx))
