...
def generate_inspirations(all_articles):
    ...
    def wangba_style(topic, source, title, idx=0):
        """网吧信息差风格 — 大学生视角荒诞解构，兄弟口吻，短平快"""
        k = keyword(topic)[:12]; s = short(topic, 30)
        p = ['夺冠','首胜','暖心','治愈','逆袭','好消息','冠军','金牌','拿奖','考入','上岸']
        n = ['偷税','被罚','暴跌','抛弃','事故','造假','曝光','投诉','崩盘','封禁','被拘','判刑']
        ip = any(w in topic for w in p); io = any(w in topic for w in n)
        if io:
            patterns = [
                f"来，这个事巴沙必须说两句：{s}。刷到的时候我还以为看错了——结果查了三遍，是真的。兄弟们你们品一品这个逻辑，开头你觉得离谱，中间发现更离谱，到最后你都不知道该骂谁。底下有没有懂的兄弟？评论区说说。",
                f"我知道你们今天肯定刷到那条了——{s}。巴沙第一反应：这什么鬼。结果越看越不对劲，怎么每层都有问题？关键是那个细节——不说了，懂的都懂。你们自己品。",
                f"今天这事有点意思。{s}。一开始以为是A，过了两天变B，第三天官方回应直接变C。连续剧都不敢这么编。评论区已经吵起来了，站哪边的都有，你们怎么看？",
                f"兄弟们，巴沙刚看到这条：{s}。最离谱的不是事情本身，是爆出来之后连锁反应。一个比一个精彩。谁对谁错我不站，但这事绝对值得吃个瓜。",
            ]
        elif ip:
            patterns = [
                f"今天巴沙要讲一个让所有人竖大拇指的事：{s}。说真的，巴沙确认了好几遍才敢发——因为太牛了怕你们不信。但这种好事多来点，我第一个点赞。评论区刷波懂你意思。",
                f"看到这条新闻巴沙直接起立：{s}。不是吹，这种反转比任何剧本都好看。人家是真干了实事。这种该火，多来点这种。",
                f"今天这条真得给大伙发一下：{s}。平时看惯了负能量，偶尔来这么一条真的舒服。普通人能做到这份上确实不容易。巴沙先点个赞。",
            ]
        else:
            patterns = [
                f"说到最近的事，巴沙必须先问一句：{s}。第一反应这什么段子，结果来真的。你们有没有那种感觉——明明每步逻辑都说得通但合在一起怎么看怎么不对劲。评论区聊五毛钱的。",
                f"今天聊一个事：{s}。表面看平平无奇，往深了捋信息量巨大。巴沙把时间线列一下：一开始——然后——现在。你发现没，跟之前那几个事连起来看，道道就出来了。",
                f"这么说吧，{s}这事巴沙刷了三遍才看懂。不是巴沙笨，是这新闻本身就绕。但绕归绕，核心就一句话：关键信息藏在中间。评论区有没有课代表总结一下。",
                f"热点来了。{s}。巴沙不多说，说多了都是泪。你们自己品。网上已经两派了，一派说对一派说不对。巴沙觉得都对也都不对——这问题本身就不该非黑即白。你们说呢？",
            ]
        return pick(patterns, topic + 'wb_v6' + str(idx))

    def aqi_style(topic, source, title, idx=0):
        """阿七纪录片风格 — 日更热点聚合，信息差视角，事件串联"""
        k = keyword(topic)[:15]; s = short(topic, 40); ts = today_str
        patterns = [
            f"{ts}社会热点信息差。先说这条——{s}。很多人只看标题就划走了，但这里面至少三个信息点值得停下来：第一，时间线比报道的早一周；第二，涉事方回应的方式有讲究；第三，受影响的不止表面这些人。每天用信息差视角看世界，你会看到不一样的。",
            f"热点信息差。{s}这个事今天几乎全网在刷，但不同平台版本差得离谱。微博说A，知乎说B，评论区老哥说C。查了一圈原始资料发现——三个版本各对了一半。真正完整的逻辑是这样。这就叫信息差。",
            f"{ts}第一个值得讲的事：{k}。看起来是普通新闻，但你把它跟前两天那条还有上周那条串起来看——三件事指向同一个方向但没人放一起。巴沙替你做了。这才是看新闻的正确姿势。",
            f"为什么{s}？因为大家只看了结果没看过程。翻一下时间线：最早的消息是——然后——到了今天才——。中间每一个节点都有人选择性地忽略了一些信息。记住，看新闻永远要看谁在说，对谁说，为什么现在说。",
            f"讲一个今天全网在讨论但没人讲清楚的事：{s}。先声明不站任何立场，只帮你理清关键事实。第一点——第二点——第三点。信息给你了，怎么用是你的事。",
        ]
        return pick(patterns, topic + 'aqi_v6' + str(idx))

    def chen_style(topic, source, title, idx=0):
        """陈先生风格 — 商业视角，数据说话，直接不绕弯"""
        k = keyword(topic)[:15]; s = short(topic, 35)
        ib = any(w in topic for w in ['上市','降价','新品','发布','收购','手机','车','股','芯片','AI','裁员','融资'])
        if ib:
            patterns = [
                f"这波真的不讲武德。{s}。直接说核心：以前行业惯例是——但现在人家直接一步到位跳过中间商。看三组数据：成本端降了、体验端升了、用户端涨了。不是吹，用数字说话。别家靠减配，这是实打实的路线。",
                f"关注一下这个事：{s}。不扯虚的，直接列事实：第一、第二、第三。很多人说不可能，因为以前——。但这次逻辑不一样：人家不是从存量里抢，是直接开了新赛道。",
                f"说到{s}，翻了一下公开数据——趋势很明显。去年这个数是X，今年直接跳到Y。背后原因是整个产业链在重构。普通人看到的是新闻，行内人看到的是信号。",
            ]
        else:
            patterns = [
                f"大型纪录片之《{k}》持续为您播出。{s}。说真的这事从来不分时间地点，什么时候来一下什么时候尴尬一下。开头你以为是A，中间发现是B，到结尾直接C了。网友评论已经炸了，我挑三条最离谱的贴在评论区。",
                f"今天看到一条挺有意思。{s}。你说它算新闻吧不算，不算吧又挺有代表性。注意到的不是事情本身，是事后各方反应——太有戏了。甲方说——乙方说——吃瓜群众说——。真实的中国互联网生态。",
                f"来，今天说一个现象：{s}。以前大家觉得是小概率事件，但现在越来越多人发现根本不是小概率，是没人统计。为什么？因为统计出来了就不好看了。点到即止，懂的都懂。",
            ]
        return pick(patterns, topic + 'chen_v6' + str(idx))

    def guancha_style(topic, source, title, idx=0):
        """人类观察菌风格 — 热点快报，客观呈现，留讨论空间"""
        k = keyword(topic)[:15]; s = short(topic, 40)
        patterns = [
            f"今日热点快报：{k}。先说基本事实——{s}。然后各方反应：官方说——当事人说——网友分两派。有意思的是中间有个细节被大多数人忽略了。原文贴在下面，你们自己看。今天这题我不给答案，评论区交作业。",
            f"热点快报，先看这条：{s}。我看了三遍才确定这不是段子。还原经过：有人——然后——最后——。现在问题是：这波到底谁该负责？评论区已经吵起来了，有人说——也有人说——。你怎么看？",
            f"今天这条热乎的：{s}。说几个注意到的点：一是——二是——三是——。其他报道基本只提了第一条。查了背景资料发现这事没那么简单。不站队只呈现，交给你们判断。",
            f"热点观察。{k}。{s}。目前网络舆论大致：30%说A有道理，40%说B活该，30%在吃瓜。我个人的观察——事情本身可能没那么重要，重要的是这事折射出的那个问题。你们觉得呢？",
            f"快报时间。{s}。根据公开信息整理时间线：开始于——发展——转折点在——现状是——。信息给你们了，不多评价。评论区聊聊你的信息差。",
        ]
        return pick(patterns, topic + 'gc_v6' + str(idx))

    def shadi_style(topic, source, title, idx=0):
        """沙漠一之雕风格 — B站博主，快节奏逐条连播，信息密集"""
        k = keyword(topic)[:12]; s = short(topic, 45); ts = today_str
        patterns = [
            f"一夜之间发生了啥？{ts}热点快报，先唠第一个。{s}。起因——发展——反转来了。你以为完了？没过多久又出来后续。网上已经吵成两派，一派说——另一派说——。来评论区一人一句，看站哪边多。",
            f"{ts}热点开唠。昨晚到今天全网最热闹的事：{k}。给没看过的朋友一句话总结——{s}。然后有意思的：前因——经过——最后结果你绝对猜不到。看了不下五个版本报道，每个版本都藏了不同事实。这就叫信息差。",
            f"两分钟补完今天热搜：先说第一个——{k}。{s}。现在的情况是——但往前翻时间线会发现——等下看评论区会很有意思。来下一条——",
            f"来，今天的热点串一下：{s}。实话实说，第一次看到我也觉得一定是编的。但查完发现——是真的，而且比编的精彩。细节是这样——别说出去，懂的都懂。下一条继续——",
            f"补一下今天热搜。{s}。就说三个时间节点：上午——下午——晚上——。情况一直在变，版本一直在变。划重点：看最早报道和最新报道差了多少东西。这就是为什么你需要信息差。",
        ]
        return pick(patterns, topic + 'sd_v6' + str(idx))
