...
def generate_inspirations(all_articles):
    ...
    def wangba_style(topic, source, title, idx=0):
        k = keyword(topic)
        s = short(topic, 30)
        pos_words = ['夺冠','首胜','治愈','暖心','逆袭','好消息','冠军','金牌','拿奖','纪录']
        neg_words = ['偷税','被罚','暴跌','抛弃','事故','争议','造假','曝光','投诉','崩盘','封禁']
        is_pos = any(w in topic for w in pos_words)
        is_neg = any(w in topic for w in neg_words)
        if is_pos:
            patterns = [f"说到今天，巴沙先说一个让所有人都竖大拇指的事：{s}。各位，这新闻刚出来的时候，巴沙刷到第一反应——不是吧，真的假的？结果确认三遍后——是真的，而且比想象中还牛。巴沙给你们细说一下前因后果：一开始——然后——最后——你说离谱不离谱？但这回的离谱不是坏事，是真排面。评论区刷个懂你意思，让巴沙看看有多少人跟我一样激动。"]
        elif is_neg:
            patterns = [f"说到最近，巴沙跟你们好好唠一个事：{k}。巴沙刷到这新闻的时候人都傻了——这谁想得到啊？你品你细品：开头你以为很离谱，中间发现比你想的还离谱，结尾——巴沙没话说了。哇也是，前两天巴沙在转转卖了个闲置小音响没想价格还挺高，有旧数码的兄弟别犹豫。那你们怎么看？评论区交给你们。"]
        else:
            patterns = [f"说到最近，巴沙先说一个事：{s}。第一反应：编的吧？然后一查——来真的。表面看平平无奇，往深了挖——水可深了。巴沙也不是很懂，懂的兄弟评论区讲讲。哇也是，前两天巴沙换了个新机，旧的在转转上挂了半天就被秒了，价格比预期高不少。有闲置数码的别放着。那最后——评论区你们唠。"]
        return pick(patterns, topic + 'wangba_v4' + str(idx))

    def aqi_style(topic, source, title, idx=0):
        k = keyword(topic)
        s = short(topic, 40)
        patterns = [
            f"热点信息差，为什么{s}？这么说吧，这事的背景比你想象的要深。一开始大家以为只是普通的——后来逐渐不对劲——现在——完全变了味。中间有一个关键细节：把时间线拉回到最开始，每一步都踩在点上。OK，这条如果觉得有用，转给身边还没注意到的人。",
            f"热点信息差，{today_str}第一个要讲的事：{k}。很多人只看标题就划走了，但信息差才是关键。先说开头——第一次转折发生在——最后结果是——三条线交叉，看清逻辑。关注我，每天用信息差视角看世界。",
        ]
        return pick(patterns, topic + 'aqi_v4' + str(idx))

    def chen_style(topic, source, title, idx=0):
        k = keyword(topic)
        s = short(topic, 35)
        is_biz = any(w in topic for w in ['上市','降价','新品','发布','收购','手机','车','股'])
        if is_biz:
            patterns = [f"这波真的不讲武德。{s}。以前我们觉得——但如今直接解锁——我们看数据：第一组——第二组——第三组告诉你值不值。别家靠减配，这是实在路线。外观——性能——典型的越卖越用心。不玩虚的。"]
        else:
            patterns = [f"大型纪录片之《{k}》持续为您播出。{s}，这事从来不分时间不分地点，心情好了来一下，心情差了来十下。开头你以为是A，中间发现是B，最后——原来是C。网友表示：这才是真正的——啊。"]
        return pick(patterns, topic + 'chen_v4' + str(idx))

    def guancha_style(topic, source, title, idx=0):
        k = keyword(topic)
        s = short(topic, 40)
        patterns = [
            f"今日热点信息快报。先看这条——{s}。开头以为普通事件——但对话直接让所有人沉默了。现在是谁讲谁有理，讲不通的。那么有意思的来了：你说他图啥？评论区分两派，一边活该一边冤枉，站哪边取决于你代入的是谁。你怎么看？",
            f"今日热点快报：{k}。我看了三遍才敢信这不是段子。还原现场：{s}。现在问题来了——这波谁的问题？评论区一人一个看法，我挑三条置顶。",
        ]
        return pick(patterns, topic + 'guancha_v4' + str(idx))

    def shadi_style(topic, source, title, idx=0):
        k = keyword(topic)
        s = short(topic, 45)
        patterns = [
            f"一夜之间发生了啥？{today_str}热点快报，先唠第一件事。{s}。起因——结果——好家伙，后面还有反转你知道吗？起因以为很离谱了，万万没想到后手更绝。没过多久——来了——网友已经在评论区整活了，一人一句看看你们怎么说。",
            f"{today_str}热点快报开唠。第一个瓜：{k}。说大不大说小不小，但我越扒越有东西。先看开头——中间——结尾你会发现比想的精彩一百倍。再串起前阵子几个热搜看，你会发现——来，下一条——",
        ]
