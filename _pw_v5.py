# -*- coding: utf-8 -*-
"""
Playwright v5 - aggressive scroll + longer wait for unauthenticated Douyin pages
Key insight from debug: page loads correctly (shows username, 589 works) but
video list (postList li) stays at 0 - videos need aggressive scrolling to load
"""
...
async def extract_videos(page):
    """Extract video data with multiple strategies"""
    videos = []
    ...
    dom_results = await page.evaluate("""() => {
        ...
        // Method 2: Any <li> inside the main content area that contains video links
        if (items.length === 0) {
        ...
        for (var el of items) {
            ...
            // Look for stat spans with data-e2e attributes
...
async def scrape_blogger(page, name, sec_uid):
    ...
    try:
        ...
        body_preview = await page.evaluate("() => document.body.innerText.substring(0, 100)")
        if name not in body_preview and "信息差" not in body_preview and "观察" not in body_preview and "纪录" not in body_preview:
            print(f"  WARNING: Page may not have loaded correctly. Body preview: {body_preview[:80]}")

        # Click on "作品" tab to ensure we're on the right view
        clicked_tab = await page.evaluate("""() => {
            var tabs = document.querySelectorAll('[class*="tab"], [data-e2e*="tab"]');
            for (var t of tabs) {
                var text = t.textContent.trim();
                if (text === '作品' || text === '视频' || text === '喜欢') {
                    t.click();
                    return text;
                }
            }
        ...
        prev_count = 0
        for scroll_round in range(10):  # Up to 10 rounds of scrolling
            # Scroll down progressively more each round
            scroll_amount = 2000 + scroll_round * 500
            await page.evaluate(f"window.scrollBy(0, {scroll_amount})")
            await page.wait_for_timeout(3000)

            # Check how many videos we can find now
            ...
            if video_links == prev_count and scroll_round > 3:
                # No progress, try mouse wheel simulation
                await page.evaluate("""() => {
        ...
        # If still nothing after all scrolls, try one more approach:
        # scroll back to top then slowly scroll down while monitoring
        if video_links == 0:
            print(f"  Trying slow scroll approach...")
        ...
        for v in videos:
            ...
            print(f"  [{len(results)}] id={vid} date={date_parsed} likes={v.get('likes',0)} "
                  f"c={v.get('comments',0)} | {v.get('title','')[:50]}")
...
async def main():
    ...
    output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '_scrape_v5_results.json')
